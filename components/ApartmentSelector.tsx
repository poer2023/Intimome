    ...
    <div className="space-y-3">
      <div className="flex justify-between items-center mb-2">
        <label className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center gap-2">
          <Home size={14} className="text-indigo-600" /> {t.where}
        </label>
      </div>
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all duration-200 group ${isSelected
                ? 'bg-indigo-50 border-indigo-500 text-indigo-700 ring-1 ring-indigo-500 shadow-sm transform scale-[1.02]'
                : 'bg-white border-slate-200 text-slate-500 hover:border-indigo-200 hover:bg-slate-50 hover:shadow-sm'
                }`}
            >
