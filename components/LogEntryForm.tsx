...
export const LogEntryForm: React.FC<LogEntryFormProps> = ({ onSave, onCancel }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newLog: SessionLog = {
      ...
      type,
      partnerName: type === ActivityType.PARTNER ? (partnerName || 'Partner') : undefined,
      location,
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {type === ActivityType.PARTNER && (
            <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center gap-2">
              <Heart size={14} className="text-emerald-600" /> {t.partnerName}
            </label>
            <input
              type="text"
              value={partnerName}
              ...
              className="w-full bg-white border border-slate-200 rounded-xl p-3.5 text-sm text-slate-800 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none shadow-sm"
            />
          </div>
          )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {Object.values(PositionType).map((pos) => {
                ...
                <button
                    ...
                    className={`group relative overflow-hidden rounded-xl transition-all duration-200 flex flex-col text-left h-40 ${
                        isSelected 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex items-end">
                <button
                    ...
                    className={`flex items-center gap-3 px-6 h-10 rounded-lg border transition-all w-full justify-center font-semibold text-sm ${
                        orgasmReached 
